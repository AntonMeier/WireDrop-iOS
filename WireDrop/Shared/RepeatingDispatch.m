...
- (void)start;
{
    WCLog(@"Resuming repeating block");
    
    if (!self.stopped)
    {
        WCLog(@"Timer is already running. No need to start new");
        return;
    }
    
    __weak typeof(self) weakSelf = self;
    
    WCLog(@"Creating repeat block.");
...
- (void)stop;
{
    WCLog(@"Cancel repeat block if needed");
