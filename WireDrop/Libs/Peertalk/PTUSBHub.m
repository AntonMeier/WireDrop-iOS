...
static usbmux_packet_t *usbmux_packet_alloc(uint32_t payloadSize) {
    //WCLog(@"Alloc size: %ld", payloadSize);
...
+ (PTUSBHub*)sharedHub {
    ...
    dispatch_once(&onceToken, ^{
        ...
        WCLog(@"shared hub listen on queue");
        [gSharedHub listenOnQueue:dispatch_get_main_queue()/*dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0)*/ onStart:^(NSError *error) {
            if (error)
            {
                WCLog(@"PTUSBHub failed to initialize: %@", error);
            }
            else
            {
                WCLog(@"PTUSBHub successfully initialized");
            }
        } onEnd:nil];
    });
    return gSharedHub;
...
- (void)connectToDevice:(NSNumber*)deviceID port:(int)port onStart:(void(^)(NSError*, dispatch_io_t))onStart onEnd:(void(^)(NSError*))onEnd {
    ...
    if (![channel openOnQueue:dispatch_get_main_queue()/*dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0)*/ error:&error onEnd:onEnd]) {
        onStart(error, nil);
        return;
    }
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
    ...
    if ([@"Attached" isEqualToString:messageType]) {
        ...
        //WCLog(@"Warning: Unhandled broadcast message: %@", packet);
    }
}
...
- (void)scheduleReadPacketWithBroadcastHandler:(void(^)(NSDictionary *packet))broadcastHandler {
    ...
    [self scheduleReadPacketWithCallback:^(NSError *error, NSDictionary *packet, uint32_t packetTag) {
        ...
        if (packetTag == 0) {
            // Broadcast message
            //WCLog(@"Received broadcast: %@", packet);
            ...
            if (requestCallback) {
                ...
                //WCLog(@"Warning: Ignoring reply packet for which there is no registered callback. Packet => %@", packet);
            }
        }
